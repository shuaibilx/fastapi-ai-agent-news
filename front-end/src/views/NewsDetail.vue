      ...
      <section class="ai-summary">
        <div class="ai-summary-header">
          ...
          <van-button
            ...
            :disabled="!newsStore.newsDetail.id"
            @click="requestSummary"
          >
            {{ summary ? '重新生成' : '生成摘要' }}
          </van-button>
        </div>
        <p v-if="summary" class="ai-summary-content">{{ summary }}</p>
        <p v-if="summary" class="ai-summary-status">
          {{ summaryCacheStatus === 'hit' ? '来自缓存' : summaryCacheStatus === 'miss' ? '刚刚生成' : '缓存暂不可用' }}
        </p>
        <p v-if="summaryError" class="ai-summary-error">{{ summaryError }}</p>
      </section>
...
import { getNewsSummary } from '../api/ai'
...
const requestSummary = async () => {
  if (!userStore.getLoginStatus) {
    showToast({ message: '请先登录后再生成摘要', position: 'bottom' })
    router.push('/login')
    return
  }
  ...
  summaryError.value = ''
  try {
    const response = await getNewsSummary(newsStore.newsDetail.id, userStore.token)
    if (response.code !== 200) {
      throw new Error(response.message || '摘要生成失败')
    }
    summary.value = response.data.summary
    summaryCacheStatus.value = response.data.cacheStatus
  } catch (error) {
    summaryError.value = error.response?.data?.message || error.message || '摘要生成失败，请稍后重试'
  } finally {
